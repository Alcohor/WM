...
instance.interceptors.response.use(response => {
  if (response && Enums.ErrorCode.CookieInvalid === response.data.code) {
    if (+localStorage.getItem('loginTryNum')) {
      location.href = '/login';
      localStorage.removeItem('loginTryNum');
      return response.data;
    } else {
      // http://wiki.iyunxiao.com/pages/viewpage.action?pageId=263718077
      location.href = `http://a.iyunxiao.com/${feConfig.appId}?go=${location.href}`;
      localStorage.setItem('loginTryNum', 1);
    }
  }
  return response.data;
}, (err) => {
